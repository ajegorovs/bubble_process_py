...
for t_conn,t_dict in t_conn_121_zp_contour_combs.items():
    ...
    for t_node in t_nodes_composite:
...
if 1 == 1:
    ...
    print(f'Working on joining all continious 121s together: {yoo}')
    # for_graph_plots(G)         <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    # NOTE: 20.09.23 zero path are not included into process, but there is nothing to interpolate
    # NOTE: drop segments that are purely ZP by checking len    see t_zero_path_segment_IDs
    # NOTE: drop "contested" nodes from interpolation data .    see t_temp
    if 1 == 1: # for_graph_plots(G)         <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        lr_big121s_interpolation = defaultdict(dict)
        lr_big121s_interpolation_big = defaultdict(dict) 

        #t_zero_path_segment_IDs = sum([list(t) for t in t_conn_121_zp_contour_combs.keys()],[]) # zp segment IDs
        ## gen only nodes that are contested
        #t_temp = [(t_time, t) for t_dict in t_conn_121_zp_contour_combs.values()                # {263: [4, 7], 264: [6]}
        #                            for t_time, t_subIDs in t_dict.items() if len(t_subIDs) > 1 # 263: [4, 7]
        #                                for t in t_subIDs ]                                     # (263,4), (263,7)
        
        for t,t_subIDs in enumerate(t_big121s_edited):
            if t_subIDs != None: #and len(set(t_subIDs) - set(t_zero_path_segment_IDs)) > 0: # questionable. solo zp will fail, zp + real will be len 1+
                t_temp_nodes_all = []
                for t_subID in t_subIDs:
                    t_temp_nodes_all += segments2[t_subID]

                #t_temp_nodes_all = [t for t in t_temp_nodes_all if t not in t_temp]       # dropped zp
                ...
                for t_edge in zip(t_subIDs[:-1], t_subIDs[1:]):
                    ...
                    #if t_edge not in t_conn_121_zp_contour_combs:
                    #    t_from,t_to = t_edge
                    #    t_start,t_end  = (segments2[t_from][-1][0], segments2[t_to][0][0])
                    #    t_times = list(np.arange(t_start + 1,t_end,1))
                    #else:
                    #    t_times = [t_time for t_time,t_subIDs in t_conn_121_zp_contour_combs[t_edge].items() if len(t_subIDs) > 1]
    ...
    if 1 == 1: # for_graph_plots(G)         <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        for t_ID in t_merge_fake_to_ID:
            ...
            #for t_perms in seqs:
            #    for t_time,t_perm in zip(t_times[t_conn],t_perms):
            #        # seqs contains copies, should have provided a list of composite nodes, but just check if calculated. for now.
            #        if not (t_conn in lr_big121s_perms_areas and t_time in lr_big121s_perms_areas[t_conn] and t_perm in lr_big121s_perms_areas[t_conn]):
...
if  1 == 1:
        ...
        for t_ID,t_param_dict in t_fake_events_k_s_edge_master.items():
            ...
            for t_ID_2 in t_IDs_remove + [t_to]:
                set_ith_elements_multilist(t_ID_2, [[],{},{},{}], t_segments_new, t_segments_121_centroids, t_segments_121_areas, t_segments_121_mom_z)

            if t_state == 'merge':   
                lr_conn_merges_to_nodes.remove(t_ID) # its not real, remove
            else:
                lr_conn_splits_from_nodes.remove(t_ID)
...
for t_ID, t_conns in t_merge_real_dict.items():
    t_branches = t_event_start_end_times[t_state][t_ID]['branches']
    for t_branch_ID in t_branches:
        t_conn = (t_branch_ID, t_ID)
        t_t_from    = t_event_start_end_times[t_state][t_ID]['t_start'][t_branch_ID]
        t_t_to      = t_event_start_end_times[t_state][t_ID]['t_end']
        ...
        t_combs_all = t_event_start_end_times[t_state][t_ID]['t_perms'][t_conn][1:]
        t_times_all = t_event_start_end_times[t_state][t_ID]['t_times'][t_conn][1:]
        ...
        for t_time, t_permutations in zip(t_times_all,t_combs_all):
            ...
            if t_diff_norms[t_where_min] < max(t_mean, 5) + 5* t_std:
                ...
                print(f'segment {t_branch_ID_new}->{t_ID} (end: {t_segments_new[t_branch_ID_new][-1]}) extended trajectory to times: {t_times_accumulate_resolved}')
                break

            if t_time == t_t_to - 1:
                print(f'segment {t_branch_ID_new}->{t_ID} (end: {t_segments_new[t_branch_ID_new][-1]}) extended trajectory to times: {t_times_accumulate_resolved}. Recovered whole inter-segment!')
...
if 1 == 1:
    for t_to,t_conns in lr_conn_merges_good.items():
        for t_conn in t_conns:
            ...
            for t_node in t_nodes_new:
                t_time,*t_subIDs = t_node# = tuple([t_time] + t_subIDs)
                t_hull = cv2.convexHull(np.vstack([g0_contours[t_time][subID] for subID in t_subIDs]))
                t_centroid, t_area, t_mom_z = centroid_area_cmomzz(t_hull)
                set_ith_elements_multilist_at_depth([t_from_new,t_node], [t_centroid,t_area,t_mom_z], t_segments_121_centroids,t_segments_121_areas,t_segments_121_mom_z)
