...
#mainOutputSubFolders= ['Field OFF Series 7','sccm350-meanFix', "00001-05000"]    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
#mainOutputSubFolders= ['HFS 265 mT Series 4','sccm300-meanFix', "00001-05000"]   # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
mainOutputSubFolders =  ['HFS 200 mT Series 4','sccm200-meanFix', "00001-05000"] 
#mainOutputSubFolders =  ['VFS 125 mT Series 5','sccm100-meanFix', "00001-05000"] # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
maxStepsForTest = -1                                                         
...
processDataSaveArchiveOnly = 0
if processDataSaveArchiveOnly: process, saveImages, saveArchive, loadArchive = 1, 0, 1, 0

processDataSaveArchiveDrawImages = 0
if processDataSaveArchiveDrawImages: process, saveImages, saveArchive, loadArchive = 1, 1, 1, 0

processArchiveAnalyzeNoImages = 0
if processArchiveAnalyzeNoImages: process, saveImages, saveArchive, loadArchive = 1, 0, 1, 0

loadDataAnalyze = 0
if loadDataAnalyze: process, saveImages, saveArchive, loadArchive = 0, 0, 0, 1
...
#==============================================================================================
if 1 == 1:
    def pltA3(IDs, preMergeMinTime,postMergeMaxTime,miltipl = [1,1,1]):
        #IDs = [15,17,19]
        areasDics = [g_areas_hull[ID] for ID in IDs]
        #for s in miltipl:
        #    for i in [0,1,2]:
        #        areasDics[i] = {t:s*val for t,val in areasDics[i].items()}
        #preMergeMinTime = 112
        #postMergeMaxTime = 137
        areasDics2 = [{t:val for t,val in areasDics[i].items() if t > preMergeMinTime} for i in [0,1]] + \
                     [{t:val for t,val in areasDics[2].items() if t < postMergeMaxTime}]

        _, axes = plt.subplots(1, 1, figsize=(6, 6), sharex=True, sharey=False)
        lw = 4
        for i in [0,1,2]:
            axes.plot(np.array(list(areasDics2[i].keys())),list(areasDics2[i].values()), label = f"A ({IDs[i]})",lw = lw)
        valSum = miltipl[0]*np.array(list(areasDics2[0].values())) + miltipl[1]*np.array(list(areasDics2[1].values()))
        
        if all([1 if i == 1 else 0 for i in miltipl]):
            label = f"A ({IDs[0]}) + A ({IDs[1]})"
        else:
            label = f"{miltipl[0]}*A ({IDs[0]}) + {miltipl[1]}*A ({IDs[1]})"
        axes.plot(np.array(list(areasDics2[0].keys())),valSum, '--', label = label,lw = lw)
        fntsize = 14
        axes.set_xlabel('laika solis',fontsize=fntsize)
        axes.set_ylabel('laukums, pix^2',fontsize= fntsize)
        axes.set_ylim(bottom=0, top = max(list(areasDics2[2].values()))+ 2000)
        axes.legend(prop={'size': 12},loc='lower right')
        axes.set_title(f"{IDs[:-1]} -> {IDs[-1]} ")
        axes.grid()
        axes.tick_params(axis='both', which='major', labelsize=12)
   
        
    pltA3([15,17,19],112,137)
    pltA3([20,21,23],165,183)
    pltA3([20,21,23],165,183,[1,0.5,1])
    plt.show()
if 1 == 1:
    ...
    resolvedID = []
    allIDs  = {ID:[] for ID in g_Centroids.keys()}                
    allIDsTs = {ID:{} for ID in g_Centroids.keys()}
    for time,dic in g_merges.items():
        for ID, subIDs in dic.items():
            if ID in allIDs:
                allIDs[ID] += subIDs                          # bubble ID and merge companion IDs
                for subID in subIDs:                          # bubble ID and merge time with companion
                    allIDsTs[ID][subID] = time

    allIDsClean     = {ID:sorted(set(subIDs)) for ID,subIDs in allIDs.items() if len(subIDs)>0}     # remove non interacting bubbles    
    allIDsTsClean   = {ID:allIDsTs[ID] for ID in allIDsClean}                                       # same about times

    allIDsTsCleanBoth = {}

    for ID, rels in allIDsTsClean.items():                                                           # OG holds only {8: {5: 109, 6: 109},..}
        for subID, time in rels.items():                                                                 # but not {5:{...},...}
            if subID not in allIDsTsCleanBoth: allIDsTsCleanBoth[subID] = {}
            allIDsTsCleanBoth[subID][ID] = time

    allIDsTsCleanBoth = {**allIDsTsClean,**allIDsTsCleanBoth}
    allIDsTsCleanBoth = dict(sorted(allIDsTsCleanBoth.items()))

    connections = []

    for mainID,subIDs in allIDsClean.items():
        for subID in subIDs:
            if mainID in allIDs and subID in allIDs:
                connections.append([mainID,subID])

    unique_cc = graphUniqueComponents(allIDs,connections)

    sizes = np.array(list(map(len,unique_cc)))

    numMerges = 2
    whereBig = np.argwhere(sizes > numMerges).reshape(-1).astype(int)

    blank = np.uint8(cv2.imread(imageLinks[0],1))*0 +255

    trajSavePath0 = [mainTestFolder] + [mainOutputSubFolders[0]] + [mainOutputSubFolders[1]+postfix] + ['trajectories_'+mainOutputSubFolders[-1]]
    trajSavePath_fam =  trajSavePath0 + ['families']
    trajSavePath_all = trajSavePath0 + ['all']


    for folderName in [trajSavePath0, trajSavePath_fam, trajSavePath_all]:
        if not os.path.exists(os.path.join(*folderName)): os.mkdir(os.path.join(*folderName))

    trajSavePath        = os.path.join(*trajSavePath0)
    trajSavePath_fam    = os.path.join(*trajSavePath_fam)
    trajSavePath_all    = os.path.join(*trajSavePath_all)
    for index in tqdm(whereBig):
        blank = blank*0 +255
        IDs = sorted(unique_cc[index])
        tmin, tmax = 2000000, 0
        for enum,ID in enumerate(IDs):

            times = list(g_areas_hull[ID].keys())
            tmin = min(tmin,min(times))
            tmax = max(tmax,max(times))

            trajectory =  np.array([g_Centroids[ID][t] for t in times ]).reshape(-1, 1, 2)
            [cv2.drawContours(  blank,   [g_contours_hull[ID][t]], -1, cyclicColor(enum), 2) for t in times[::20]]
            #cv2.polylines(blank, [trajectory] ,0, (255,255,255), 3)
            cv2.polylines(blank, [trajectory] ,0, cyclicColor(enum), 3)
            [cv2.circle(blank, tuple(p), 1, cyclicColor(enum), -1) for [p] in trajectory]
        filename = f't_{tmin}-{tmax}_IDS-'+"-".join(map(str,IDs))+".png"
        cv2.imwrite(os.path.join(trajSavePath_fam,filename) ,blank)

        #cv2.imshow(f'asd', resizeToMaxHW(blank))
    a = 1
    whereSolo = np.argwhere(sizes == 1).reshape(-1).astype(int)
    for index in tqdm(whereSolo):
        ID = unique_cc[index][0]
        times = list(g_areas_hull[ID].keys())

        trajectory0 =  {t:g_Centroids[ID][t] for t in times if 1110 > g_Centroids[ID][t][0] > 50}
        times = list(trajectory0.keys())
        

        trajectory =  np.array( list(trajectory0.values())).reshape(-1, 1, 2)
        if len(trajectory) < 5: continue
        blank = blank*0 +255
        step = min(20,len(times)-1)
        [cv2.drawContours(  blank,   [g_contours_hull[ID][t]], -1, [175,133,33], -1) for t in times[::step]]
        [cv2.drawContours(  blank,   [g_contours_hull[ID][t]], -1, [145,83,0], 3) for t in times[::step]]
        #cv2.polylines(blank, [trajectory] ,0, (255,255,255), 3)
        cv2.polylines(blank, [trajectory] ,0, [0,0,0], 3)
        [cv2.circle(blank, tuple(p), 4, [0,0,0], -1) for [p] in trajectory]


        tmin,tmax = min(times), max(times)
        filename = f'ID-{str(ID).zfill(4)}_t-{str(tmin).zfill(4)}-{str(tmax).zfill(4)}.png'
        cv2.imwrite(os.path.join(trajSavePath_all,filename) ,blank)


    
    blank = blank*0 +255
    for index,ID in tqdm(enumerate(allIDs)):
        ...
        #[cv2.drawContours(  blank,   [g_contours_hull[ID][t]], -1, [145,83,0], 3) for t in times[::step]]
        #cv2.polylines(blank, [trajectory] ,0, (255,255,255), 3)
        cv2.polylines(blank, [trajectory] ,0, cyclicColor(index), 3)
        [cv2.circle(blank, tuple(p), 4, cyclicColor(index), -1) for [p] in trajectory]


        tmin,tmax = min(times), max(times)
        filename = f'AllTogetherCutTopBottomNoShort.png'
    cv2.imwrite(os.path.join(trajSavePath,filename) ,blank)
    k = cv2.waitKey(0)
