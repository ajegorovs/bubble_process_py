...
def draw_graph_with_height(g,highlighted_nodes=set([]),figsize=(15,15),iterations=150,title='', labels = []):
...
# print(weights)
  ...
  if highlighted_nodes:
    nx.draw(g, pos=pos_dict, alpha=.1, font_size=14,node_color='b', labels = labels)
    gsub = nx.subgraph(g,highlighted_nodes)
    nx.draw(gsub, pos=pos_dict, node_color='r', labels = labels)

  else:
    nx.draw(g,pos=pos_dict, labels = labels)
